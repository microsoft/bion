...
namespace BSOA.Demo.Model
{
    public struct Run
    {
        ...
        {
            get => new MutableSliceWrapper<Result, ResultTable>(_table.Results[_index], _table.Database.Result, (table, index) => new Result(table, index), (item) => item._index);
            set => new MutableSliceWrapper<Result, ResultTable>(_table.Results[_index], _table.Database.Result, (table, index) => new Result(table, index), (item) => item._index).SetTo(value);
        }

        public IList<Artifact> Artifacts
        {
            get => new MutableSliceWrapper<Artifact, ArtifactTable>(_table.Artifacts[_index], _table.Database.Artifact, (table, index) => new Artifact(table, index), (item) => item._index);
            set => new MutableSliceWrapper<Artifact, ArtifactTable>(_table.Artifacts[_index], _table.Database.Artifact, (table, index) => new Artifact(table, index), (item) => item._index).SetTo(value);
        }
    }
